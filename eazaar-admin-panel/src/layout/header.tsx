...
const Header = ({ setSideMenu }: IProps) => {
  ...
  return (
    <>
      <header className="relative z-10 bg-white dark:bg-slate-800 border-b border-gray dark:border-slate-700 border-solid py-5 px-8 pr-8 transition-colors duration-300">
        <div className="flex justify-between">
          <div className="flex items-center space-x-6 lg:space-x-0">
            <button
              onClick={() => setSideMenu((prev) => !prev)}
              type="button"
              className="block lg:hidden text-2xl text-black dark:text-white"
            >
              <Menu />
            </button>
            <div className="w-[30%] hidden md:block">
              <form action="#">
                <div className="w-[250px] relative">
                  <input
                    className="input h-12 w-full pr-[45px] dark:bg-slate-700 dark:border-slate-600 dark:text-white dark:placeholder-slate-400"
                    type="text"
                    placeholder="Search Here..."
                  />
                  <button className="absolute top-1/2 right-6 translate-y-[-50%] hover:text-theme dark:text-slate-300">
                    <Search />
                  </button>
                </div>
              </form>
          ...
          <div className="flex items-center justify-end space-x-6">
            <div className="md:hidden">
              <button className="relative w-[40px] h-[40px] leading-[40px] rounded-md text-textBody dark:text-slate-300 border border-gray dark:border-slate-600 hover:bg-themeLight hover:text-theme hover:border-themeLight dark:hover:bg-slate-700">
                <Search />
              </button>
            </div>
            {/* theme toggle start */}
